...
class SBOM extends BaseCommand {
  ...
  async exec () {
    ...
    for (const node of items) {
      ...
      // Collect all the unique visited nodes (prefer links over the link target)
      const found = seenNodes.get(node.pkgid)

      /* istanbul ignore else */
      if (!found || node.isLink) {
        seenNodes.set(node.pkgid, node)
      }
    }
